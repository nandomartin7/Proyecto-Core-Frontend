<template>
    ...
    <div class="login-container">
      ...
      <form @submit.prevent="login" class="formularioInicio">
        <p><input v-model="credentials.idEmpleado" class="input-fieldInicioN" placeholder="Cedula del empleado" required /></p>
    ...
    methods: {
        ...
        login() {
            ...
            .then(response => {
                ...
                localStorage.setItem('empleado', JSON.stringify(response.data.empleado)); //Guarda el objeto empleado como JSON
                this.$router.push('/empleado/'); // Redirigir a la página principal
            })
            .catch(error => {
                if (error.response) {
